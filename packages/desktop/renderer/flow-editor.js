...
// Capsule Definitions with Ports
const CAPSULES = [
    {
        id: 'auth-jwt',
        name: 'JWT Auth',
...
function renderNode(node) {
    ...
    const inputPortsHTML = node.capsule.inputs.map((port, index) => `
        <div class="port port-input"
             ...
             title="${port.name} (${port.type.name})">
            <div class="port-dot" style="background: ${port.type.color};"></div>
    ...
    const nodeHeight = Math.max(
        70 + Math.max(node.capsule.inputs.length, node.capsule.outputs.length) * 25,
        120
    );
